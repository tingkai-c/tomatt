...
- Pairing flow:
  1. The user chooses **Pair Device…**.
  2. Nearby devices appear through LAN discovery, and **Pair by Address…** is available for manual LAN/private-overlay/Tailscale reachability.
  3. User selects the nearby device or enters the pairing address.
  4. Devices establish a WebSocket session and exchange pairing handshake metadata.
  5. Both devices show the same transcript-derived six-digit verification code.
  6. User confirms the code on both devices.
  7. Devices show a pre-merge preview before importing events.
  8. User approves the merge preview and settings-source choice on both devices.
  9. Devices exchange identity, sync-group information, membership events, and syncable events.
- Internal Add Device / Join Sync Group protocol roles may still exist for deterministic transcripts, but they are not exposed as separate top-level user actions.
- Manual IP/host/Tailscale pairing is supported in v1 through **Pair by Address…**. Manual reachability replaces discovery only; it does not bypass verification, encryption, preview approval, signed import, or membership validation.
...
- The default listener/manual pairing port is `40484`, with an advanced override if that port is unavailable or unsuitable. Bonjour advertises the actual active port.
...
- Devices retain original signed sync event metadata so a paired device can relay another origin device's events unchanged to a third paired device.
- Anti-entropy summaries and missing-event requests operate per origin device across all trusted origins, not only the connected peer or local signer.
...
- Manual IP/host/Tailscale fallback is provided in v1 through Pair by Address. It uses the same endpoint, subprotocol, verification, encryption, and trust path as Bonjour-discovered pairing.
...
- Signed events are valid only within the origin device's event-sourced membership interval. Events ordered before removal may remain valid; events ordered after removal are rejected. Wall clock alone is not used as the validity cutoff.
...
- Pair Device… with nearby discovery and Pair by Address…
- manual pairing port override and port-unavailable status
